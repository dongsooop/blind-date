...
export class SessionRepository {
  ...
  public async leave(sessionIds: Set<string>, socketId: string) {
    ...
    const socketIds = JSON.parse(socketIdsRaw || '[]') as Set<string>;
    ...
    for (const sessionId of sessionIds) {
      ...
      if (socketIds.size > 1) {
        ...
        await this.redisClient
          ...
          .del(SessionKeyFactory.getSocketKey(socketId)) // 소켓 키 삭제
          .hSet(memberKey, 'socket', JSON.stringify(socketIds))
          .exec();
      }
    }
  }
  ...
  public async addMember(
    ...
    const participants = await this.getParticipants(sessionId);
    participants.push(
      new Participant(memberId, new Set<string>(socketId), name),
    );
  ...
  public async getParticipants(sessionId: string) {
    ...
    return participantsParsed.map((p) => {
      ...
      // socketIdSet이 배열로 저장된 경우
      if (Array.isArray(p.socketIdSet)) {
        socketIds = p.socketIdSet;
      }
      // socketIdSet이 {} 형태로 저장된 경우 (Set이 stringify된 케이스)
      else if (p.socketIdSet && typeof p.socketIdSet === 'object') {
        socketIds = Object.values(p.socketIdSet) as string[];
      }

      return new Participant(p.memberId, new Set(socketIds), p.name);
    });
  }
  ...
  public async getSocketIds(memberId: number) {
    ...
    return JSON.parse(socketIdsRaw) as Set<string>;
  }
  ...
  public async getSocketIdByMemberId(sessionId: string, memberId: number) {
    ...
    return participants
      .filter((participant) => participant.equalsMemberId(memberId))
      .map((participant) => participant.getSocketIdSet())
      .flatMap((v) => [...v.values()]);
  }
  ...
  public async getAllMembers(
    ...
    return participants.reduce((acc, cur) => {
      acc.set(cur.getMemberId(), cur.getSocketIdSet());
      return acc;
